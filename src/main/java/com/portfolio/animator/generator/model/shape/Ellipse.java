...
public class Ellipse extends AShape {
  ...
  public IShape getShapeAt(int tick) {
    ...
    for (ITransformation t : this.transformations) {
      ...
      b2 = t.getFinalBlue();
      if (t1 == tick) {
        s = new Ellipse(ShapeType.ELLIPSE, name, t1, x1, y1, w1, h1, r1, g1, b1);
        return s;
      }
      else if (t2 == tick) {
        s = new Ellipse(ShapeType.ELLIPSE, this.name, t2, x2, y2, w2, h2, r2, g2, b2);
        return s;
      }
      else if (tick > t1 && tick < t2) {
        tdiff = t2 - t1;
        s = new Ellipse(ShapeType.ELLIPSE, this.name, tick, tweening(x2, x1, tdiff),
